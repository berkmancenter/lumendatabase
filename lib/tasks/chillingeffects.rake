...
where works.id in (
  ...
  desc "Re-assign Google recipients to the canonical Twitter"
  task reassign_google_entities: :environment do
      google_entities = Entity.select("id").where(:name => ["Google", "Google, Inc.", "GOOGLE INC.", "google", "Google Inc", "Twitter,Inc.", "Google USA", "google.com", "google, inc", "google inc", "google inc."]).pluck(:id)
      google_entities - [1]

...
require 'lumen/importers/youtube/mapping/html/counterfeit'
require 'lumen/importers/youtube/mapping/html/defamation'
require 'lumen/importers/youtube/mapping/html/other_legal'
require 'lumen/importers/youtube/mapping/html/trademark_d'
require 'lumen/importers/youtube/mapping/plain_new/counterfeit'
require 'lumen/importers/youtube/mapping/plain_new/defamation'
require 'lumen/importers/youtube/mapping/plain_new/other_legal'
require 'lumen/importers/youtube/mapping/plain_new/trademark_d'

module Lumen::Importers::Youtube
  class Importer
    FILES_DIRECTORY = ENV['BASE_DIRECTORY']
    IMPORT_FILE_BATCH_SIZE = 500

    def initialize
      @logger = Lumen::Logging::Loggy.new('YoutubeImporter', true, true)
    ...
    def import_single_notice(file_to_process)
      ...
      mapper_class = nil
      mapper_class = "Lumen::Importers::Youtube::Mapping::#{format_class}::Counterfeit" if system("grep -q '<counterfeit\+' #{file_to_process}")
      mapper_class = "Lumen::Importers::Youtube::Mapping::#{format_class}::Defamation" if system("grep -q '<defamation\+' #{file_to_process}")
      mapper_class = "Lumen::Importers::Youtube::Mapping::#{format_class}::OtherLegal" if system("grep -q '<other-legal\+' #{file_to_process}")
      mapper_class = "Lumen::Importers::Youtube::Mapping::#{format_class}::TrademarkD" if system("grep -q '<trademark\+' #{file_to_process}")
      ...
      new_notice = Lumen::NoticeBuilder.new(

...
class Lumen::Reports::CourtOrderReporter
  def initialize
    @loggy = Lumen::Logging::Loggy.new('rake lumen:CourtOrderReporter', true)
  end
  ...
  def email_single_user(email)
    Net::SMTP.new(SMTP_SETTINGS[:address], tls_verify: false).start do |smtp|

class CounterNotice
  ...
  def initialize(attributes = {})
    attributes.respond_to?(:keys) && attributes.keys.each do |key|
      send("#{key}=".to_sym, attributes[key]) if respond_to?("#{key}=".to_sym)
    end
  end
  ...
  attr_accessor :attach_list_of_works, :list_removed_in_error, :perjury_risk_acknowledged, :jurisdiction, :consent_to_be_served, :your_name, :your_phone, :your_address_line_1, :your_address_line_2, :your_address_line_3, :your_address_line_4, :service_provider_name, :service_provider_phone, :service_provider_address_line_1, :service_provider_address_line_2, :service_provider_address_line_3, :service_provider_address_line_4

...
describe Entity, type: :model do
  ...
  it 'does not have an index name suffix when the ENV variable is unset' do
    ...
    load 'app/models/entity.rb'
    expect(Entity.index_name).to eq 'chill_application_test_entity'
  ...
  it 'uses the index name suffix when the ENV variable is set' do
    ...
    Object.send(:remove_const, 'Entity')
    load 'app/models/entity.rb'
    expect(Entity.index_name).to eq 'chill_application_test_entity_testme'

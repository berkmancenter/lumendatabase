...
describe TokenUrlsController do
  ...
  context '#create' do
    it 'creates a new token url when valid params are provided' do
      ...
      expect(TokenUrl.last.email).to eq Lumen::Security::Hasher.hash512('user@example.com')
    end
  ...
  context '#generate_permanent' do
    ...
    context '#clean_up_email_address' do
      ...
      it 'removes periods if a new token email address is gmail' do
        ...
        expect(TokenUrl.last.email).to eq Lumen::Security::Hasher.hash512('userxxx@gmail.com')

        create_token 'user.x.x.x@googlemail.com', '2.2.2.2'

        expect(TokenUrl.last.email).to eq Lumen::Security::Hasher.hash512('userxxx@googlemail.com')
      end
    end
  end
